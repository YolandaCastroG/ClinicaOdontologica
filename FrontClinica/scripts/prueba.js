    ...
    .then((data) => {
      ...
      data.forEach((paciente, index) => {
        ...
        row.innerHTML = `
          ...
          paciente.apellido
        }', '${paciente.dni}', '${paciente.fechaIngreso}', ${JSON.stringify(
          paciente.domicilio
        )})">Modificar</button>
                                <button class="btn btn-danger btn-sm" onclick="deletePaciente(${
                                  paciente.id
                                })">Eliminar</button>
                            </td>
                        `;
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    const domicilio = { calle, numero, localidad, provincia };
